...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border p-6">
        <div class="flex justify-between items-start mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Détails de la langue</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-600">Nom de la langue</label>
                    <p class="mt-1 text-lg font-medium text-gray-900">{{ $langue->nom_langue }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-600">Code</label>
                    <p class="mt-1 text-sm text-gray-900 font-mono">{{ $langue->code_langue }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-600">Date de création</label>
                    <p class="mt-1 text-sm text-gray-900">{{ $langue->created_at->format('d/m/Y à H:i') }}</p>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                @if($langue->description)
                    <p class="text-sm text-gray-900">{{ $langue->description }}</p>
                @else
        ...
        <div class="mt-8 pt-6 border-t">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-blue-50 rounded-lg p-4 text-center">
                    <div class="text-blue-600 text-2xl font-bold">{{ $langue->contenus_count }}</div>
                    <div class="text-blue-800 text-sm">Contenus associés</div>
                </div>
                <div class="bg-green-50 rounded-lg p-4 text-center">
                    <div class="text-green-600 text-2xl font-bold">{{ $langue->utilisateurs_count ?? 0 }}</div>
                    <div class="text-green-800 text-sm">Utilisateurs</div>
                </div>
                <div class="bg-purple-50 rounded-lg p-4 text-center">
                    <div class="text-purple-600 text-2xl font-bold">{{ $langue->parlers_count ?? 0 }}</div>
                    <div class="text-purple-800 text-sm">Régions</div>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <div>
                        ...
                        <p class="text-sm text-gray-600">{{ $contenu->region->nom_region }} • {{ $contenu->created_at->format('d/m/Y') }}</p>
                    </div>
