...
@section('content')
<div class="space-y-6">
    <!-- Bande d'information de rôle -->
    <div class="bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg shadow-sm p-4">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm opacity-90">Bienvenue</p>
                <h2 class="text-xl font-bold">{{ Auth::user()->prenom }} {{ Auth::user()->nom }}</h2>
                <p class="text-sm opacity-75">Rôle: <span class="font-semibold">{{ Auth::user()->role->nom_role }}</span></p>
            </div>
            <div class="text-right text-sm opacity-75">
                @if(Auth::user()->hasRole('admin'))
                    <p class="font-semibold">Administrateur complet</p>
                    <p>Toutes les permissions</p>
                @elseif(Auth::user()->hasRole('moderator'))
                    <p class="font-semibold">Modérateur</p>
                    <p>Gestion du contenu et modération</p>
                @elseif(Auth::user()->hasRole('contributor'))
                    <p class="font-semibold">Contributeur</p>
                    <p>Création de contenu (lecture seule pour l'admin)</p>
                @endif
            </div>
        </div>
    </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        @if(Auth::user()->hasRole('admin'))
                        <a href="{{ route('admin.utilisateurs.show', $user) }}" class="text-xs text-blue-600 hover:underline">
                            Voir
                        </a>
                        @endif
                    </div>
